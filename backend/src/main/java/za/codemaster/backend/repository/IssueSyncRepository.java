...
public class IssueSyncRepository {
    public record ExistingIssue(long id, Long overriddenBy){}
    ...
    public Optional<ExistingIssue> findByProjectIdAndGithubIssueNumber(long projectId,int number){
        return jdbc.query("select id,difficulty_overridden_by_user_id from issues where project_id=? and github_issue_number=?",
            (rs,n)->new ExistingIssue(rs.getLong("id"),(Long)rs.getObject("difficulty_overridden_by_user_id")),projectId,number).stream().findFirst();
    }
